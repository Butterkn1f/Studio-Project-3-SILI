...
public:
	...
	bool SetVolume(const int ID, const float volume);

	bool SetMasterVolume(const float volume);
	bool SetBGMVolume(const float volume);
	bool SetSFXVolume(const float volume);

	bool isPlaying(const int ID);
	float getCurrentVolume(const int ID);
