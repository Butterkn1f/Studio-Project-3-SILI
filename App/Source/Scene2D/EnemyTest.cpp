 ...
 */
void CEnemyTest::Update(const double dElapsedTime)
{
	if (!bIsActive)
		return;
	...
	case PATROL:
		...
		{
			// Patrol around
			// Update the EnemyTest's position for patrol
			UpdatePosition();
		}
		iFSMCounter++;
		break;
		...
		if (cPhysics2D.CalculateDistance(vec2Index, cPlayer2D->vec2Index) < 5.0f)
		{
			...
			for (const auto& coord : path)
			{
				if (bFirstPosition == true)
				{
					...
					// Calculate the direction between EnemyTest and this destination
			...
			// Update the EnemyTest's position for attack
			UpdatePosition();
		}
		else
		{
 ...
 */
void CEnemyTest::PreRender(void)
{
	if (!bIsActive)
		return;
 ...
 */
void CEnemyTest::PostRender(void)
{
	if (!bIsActive)
		return;
...
/**
@brief Set the indices of the EnemyTest
@param iIndex_XAxis A const int variable which stores the index in the x-axis
@param iIndex_YAxis A const int variable which stores the index in the y-axis
*/
void CEnemyTest::Seti32vec2Index(const int iIndex_XAxis, const int iIndex_YAxis)
{
...
/**
@brief Set the number of microsteps of the EnemyTest
...
*/
void CEnemyTest::Seti32vec2NumMicroSteps(const int iNumMicroSteps_XAxis, const int iNumMicroSteps_YAxis)
{
 ...
 */
void CEnemyTest::SetPlayer2D(CPlayer2D* cPlayer2D)
{
...
/**
 @brief Constraint the EnemyTest's position within a boundary
 @param eDirection A DIRECTION enumerated data type which indicates the direction to check
 */
void CEnemyTest::Constraint(DIRECTION eDirection)
{
	if (eDirection == LEFT)
	{
		if (vec2Index.x < 0)
	...
	{
		cout << "CEnemyTest::Constraint: Unknown direction." << endl;
	}
}
...
/**
 ...
 @param eDirection A DIRECTION enumerated data type which indicates the direction to check
 */
bool CEnemyTest::CheckPosition(DIRECTION eDirection)
{
	if (eDirection == LEFT)
	{
	...
	{
		cout << "CEnemyTest::CheckPosition: Unknown direction." << endl;
	}

	return true;
}

// Check if the EnemyTest is in mid-air
bool CEnemyTest::IsMidAir(void)
{
...
// Update Jump or Fall
void CEnemyTest::UpdateJumpFall(const double dElapsedTime)
{
...
/**
 @brief Let EnemyTest interact with the player.
 */
bool CEnemyTest::InteractWithPlayer(void)
{
	glm::i32vec2 i32vec2PlayerPos = cPlayer2D->vec2Index;
	
	// Check if the EnemyTest is within 1.5 indices of the player2D
 ...
 */
void CEnemyTest::UpdateDirection(void)
{
	// Set the destination to the player
	i32vec2Destination = cPlayer2D->vec2Index;

	// Calculate the direction between EnemyTest and player2D
	i32vec2Direction = i32vec2Destination - vec2Index;

	// Calculate the distance between EnemyTest and player2D
 ...
 */
void CEnemyTest::FlipHorizontalDirection(void)
{
...
*/
void CEnemyTest::UpdatePosition(void)
{
	// Store the old position
	i32vec2OldIndex = vec2Index;

	// if the player is to the left or right of the EnemyTest, then jump to attack
	if (i32vec2Direction.x < 0)
	{
		...
		// Constraint the EnemyTest's position within the screen boundary
		Constraint(LEFT);

		// Find a feasible position for the EnemyTest's current position
		...
		// Check if EnemyTest is in mid-air, such as walking off a platform
		if (IsMidAir() == true)
		{
			cPhysics2D.SetStatus(CPhysics2D::STATUS::FALL);
		}
	...
	// if the player is above the EnemyTest, then jump to attack
