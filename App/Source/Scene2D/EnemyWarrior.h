...
class CEnemyWarrior : public CEntity2D
{
public:
	// Constructor
	CEnemyWarrior(void);
...
protected:
	...
	// The vec2 which stores the indices of the EnemyWarrior in the Map2D
	glm::vec2 i32vec2Index;

	// The vec2 variable which stores The number of microsteps from the tile indices for the EnemyWarrior. 
	// A tile's width or height is in multiples of these microsteps
	glm::vec2 i32vec2NumMicroSteps;

	// The vec2 variable which stores the UV coordinates to render the EnemyWarrior
	glm::vec2 vec2UVCoordinate;

	// The vec2 which stores the indices of the destination for EnemyWarrior in the Map2D
	glm::vec2 i32vec2Destination;
	// The vec2 which stores the direction for EnemyWarrior movement in the Map2D
	...
	// Constraint the EnemyWarrior's position within a boundary
	...
	// Let EnemyWarrior interact with the player
